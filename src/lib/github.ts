...
export async function getGitHubRepos(): Promise<ProcessedRepo[]> {
	try {
		const response = await fetch(
			"https://api.github.com/users/lucaserm/repos?sort=updated&per_page=50",
			{
				headers: {
		...
		const publicRepos = validatedRepos.filter(
			(repo) => !repo.private && !repo.archived && !repo.disabled,
		);
		...
		const processedRepos: ProcessedRepo[] = publicRepos.map((repo) => ({
			...
			name: repo.name,
			displayName:
				PROJECT_NAME_MAPPINGS[repo.name] || formatRepoName(repo.name),
...
export async function getGitHubRepo(
	...
	try {
		const response = await fetch(
			`https://api.github.com/repos/lucaserm/${repoName}`,
			{
				headers: {
					Accept: "application/vnd.github.v3+json",
				},
		...
		return {
			...
			displayName:
				PROJECT_NAME_MAPPINGS[validatedRepo.name] ||
				formatRepoName(validatedRepo.name),
